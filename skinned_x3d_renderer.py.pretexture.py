# --- START OF FILE skinned_x3d_renderer.py (FINAL - CORRECT MATERIAL & LIGHTING) ---
...
class InteractiveAnariRenderer:
    ...
    def _initialize_scene(self):
        ...
        # --- FIX: Revert to using the light colors from the X3D scene file ---
        ...
        for light_node in scene_lights:
            ...
            # Read the color from the X3D light node, defaulting to white if not present.
    ...
    # --- FIX: Function signature updated to accept material properties ---
    def _setup_common_geometry(self, vertices, indices, texcoords=None, sampler=None, diffuse=None, emissive=None):
        ...
        # Set the base diffuse color from the X3D Material node.
        material.setParameter('color', anari.FLOAT32_VEC3, diffuse if diffuse is not None else (1,1,1))
        # Set the emissive color from the X3D Material node.
        material.setParameter('emissive', anari.FLOAT32_VEC3, emissive if emissive is not None else (0,0,0))

        if sampler is not None:
            # Tell the material to use the sampler, which will be multiplied by the 'color' parameter above.
            material.setParameter("map.color", anari.SAMPLER, sampler)
            # Tell the material to get the UVs for the map from our generic vertex attribute.
    ...
    def _setup_hanim_skinned(self, humanoid_node):
        ...
        if shape_node:
            ...
            if hasattr(shape_node, 'appearance'):
                ...
                # --- FIX: Read Material and Texture properties from the Appearance node ---
                ...
                if hasattr(app_node, 'texture') and isinstance(app_node.texture, x3d.ImageTexture):
                    ...
                    texture_filename = tex_node.url[0] if tex_node.url else None
        ...
        # --- FIX: Pass the collected scene data to the geometry setup function ---
