...
class InteractiveAnariRenderer:
    def __init__(self, x3d_scene, width, height):
        ...
        self.start_time = time.time(); self.device = anari.newDevice('helide')
    ...
    def _initialize_scene(self):
        ...
        if not self.scene_node: raise RuntimeError("No Scene node found in the X3D file.")
        ...
        if self.viewpoint_node and hasattr(self.viewpoint_node, 'position'):
            ...
            # --- CAMERA FIX: Negate Z-axis to align X3D viewpoint with orbital camera math ---
    ...
    def _traverse(self, node):
        ...
        if not isinstance(node, x3d.meta) and type(node).__name__ == 'module':
            for name in dir(node):
                if not name.startswith('__'):
                    self._traverse(getattr(node, name))
            return

        if not self.scene_node and isinstance(node, x3d.Scene): self.scene_node = node
    ...
    def _create_anari_surface(self, positions, texcoords, indices, sampler, diffuse, emissive):
        ...
        if sampler and texcoords is not None and len(texcoords) > 0:
            ...
            # --- CHANGE 2: REMOVE this line. The link is now on the sampler. ---
        ...
        material.setParameter('metallic', anari.FLOAT32, 0.0); material.setParameter('roughness', anari.FLOAT32, 0.6)
        ...
        for node in skinned_shapes:
            ...
            if hasattr(node.geometry, 'texCoord') and node.geometry.texCoord:
                ...
                if isinstance(tex_coord_container, x3d.MultiTextureCoordinate):
                    ...
                    if target_mapping_name:
                        print(f"-> Searching for TextureCoordinate with mapping: '{target_mapping_name}'")
                        for tc_node_ref in tex_coord_container.texCoord:
                            ...
                            if getattr(tc_node, 'mapping', None) == target_mapping_name:
                                data_node = tc_node
                                print(f"->   Found matching TextureCoordinate node '{getattr(data_node, 'DEF', 'N/A')}'.")
                                found_tc_node = True
                                break

                    if not found_tc_node and tex_coord_container.texCoord:
                        if tex_node: print(f"-> WARNING: Could not find mapping '{target_mapping_name}'. Falling back to first available TextureCoordinate set.")
            ...
            if tex_node:
                ...
                if img:
                    ...
                    # --- CHANGE 1: Tell the SAMPLER which vertex attribute to use for coordinates ---
    ...
    def _setup_display(self):
        ...
        # Revert to prioritizing 'pathtracer'
        renderer_types_to_try = ["pathtracer", "scivis"]
        renderer = None
        for r_type in renderer_types_to_try:
            try:
                print(f"ANARI: Attempting to create renderer: '{r_type}'...")
                r = self.device.newRenderer(r_type)
                if r:
                    renderer = r
                    print(f"ANARI: Successfully created renderer: '{r_type}'")
                    break
                else:
                    print(f"ANARI: INFO - Renderer '{r_type}' not available or failed to create.")
            except Exception as e:
                print(f"ANARI: WARNING - Could not create renderer '{r_type}': {e}")

        if not renderer:
            raise RuntimeError("ANARI: Could not create any of the preferred renderers ('pathtracer', 'scivis').")
