# --- START OF FILE skinned_x3d_renderer.py (FINAL) ---
...
class InteractiveAnariRenderer:
    ...
    def _initialize_scene(self):
        ...
        print("--- Starting Scene Traversal ---")
        self._traverse(scene_node)
        print("--- Scene Traversal Complete ---")

        # UNTOUCHED BASELINE CAMERA LOGIC
        ...
        # UNTOUCHED BASELINE LIGHTING LOGIC
    ...
    def _traverse(self, node):
        # The robust traversal function that correctly finds all nodes.
        if not hasattr(node, '__dict__'):
            return

        node_def = getattr(node, 'DEF', None)
        # ADDED LOGGING
        print(f"Traversing: {type(node).__name__}, DEF='{node_def if node_def else 'N/A'}'")
    ...
    def _setup_common_geometry(self, vertices, indices, vertex_colors=None):
        ...
        if vertex_colors is not None:
            ...
            print(f"  First 20 vertex coordinates passed to ANARI: \n{vertices[:20]}")
            print(f"  First 20 vertex colors passed to ANARI: \n{vertex_colors[:20]}")
            color_array = self.device.newArray1D(anari.FLOAT32_VEC3, vertex_colors)
            material.setParameter('color', anari.ARRAY, color_array)
        else:
    ...
    def _setup_hanim_skinned(self, humanoid_node):
        ...
        all_indices, all_texcoords, image_url = [], None, None

        diffuse_color = np.array([1.0, 1.0, 1.0], dtype=np.float32)
        emissive_color = np.array([0.0, 0.0, 0.0], dtype=np.float32)

        material_node = self.def_map.get("Joe_skin_Material")
        if material_node:
            if hasattr(material_node, 'diffuseColor'):
                diffuse_color = np.array(material_node.diffuseColor, dtype=np.float32)
                print(f"Found material diffuseColor: {diffuse_color}")
            if hasattr(material_node, 'emissiveColor'):
                emissive_color = np.array(material_node.emissiveColor, dtype=np.float32)
                print(f"Found material emissiveColor: {emissive_color}")
        else:
            print("Warning: Could not find material 'Joe_skin_Material' via robust traversal. Using default colors.")
        ...
        while nodes_to_process:
            ...
            if isinstance(node, x3d.Shape):
                ...
                if isinstance(geom, x3d.IndexedFaceSet) and geom.coordIndex:
                    ...
                    if geom.texCoord and hasattr(geom.texCoord, 'point') and geom.texCoord.point:
                         all_texcoords = np.array(geom.texCoord.point, dtype=np.float32).reshape(-1, 2)
                if node.appearance and node.appearance.texture and isinstance(node.appearance.texture, x3d.ImageTexture):
                    if hasattr(node.appearance.texture, 'url') and node.appearance.texture.url:
                        image_url = node.appearance.texture.url[0]
            ...
            try:
                ...
                for i in range(num_verts):
                    if i < len(all_texcoords):
                        ...
                        texture_sample = img_data[y, x] / 255.0
                        final_color = emissive_color + (diffuse_color * texture_sample)
                        vertex_colors[i] = np.clip(final_color, 0.0, 1.0)
                    else:
                        vertex_colors[i] = np.clip(emissive_color + diffuse_color, 0.0, 1.0)
            except (FileNotFoundError, IOError) as e:
                print(f"Warning: Could not load texture '{image_url}'. Using material color only.")
                for i in range(num_verts):
                    vertex_colors[i] = np.clip(emissive_color + diffuse_color, 0.0, 1.0)
        else:
            print("No texture found. Using material color only.")
            for i in range(num_verts):
                vertex_colors[i] = np.clip(emissive_color + diffuse_color, 0.0, 1.0)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(texture_file):
        print(f"ERROR: Texture file '{texture_file}' not found in the current directory.")
        print("Please ensure it is present to see the textured model.")
        try:
            Image.new('RGB', (64, 64), color = 'purple').save(texture_file)
            print(f"Created a placeholder texture '{texture_file}'.")
