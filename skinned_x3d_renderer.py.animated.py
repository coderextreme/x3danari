# --- START OF FILE skinned_x3d_renderer.py (Manual UV Mapping) ---
...
class InteractiveAnariRenderer:
    ...
    def _traverse(self, node):
        ...
        for child_list in [getattr(node, attr, []) for attr in ['children', 'skeleton', 'skin']]:
            for child in child_list: self._traverse(child)
    ...
    ### MODIFIED: THIS FUNCTION NOW ACCEPTS PRE-CALCULATED VERTEX COLORS ###
    def _setup_common_geometry(self, vertices, indices, vertex_colors=None):
        ...
        else: # Fallback to a default color
    ...
    ### MODIFIED: THIS FUNCTION NOW PERFORMS THE UV->COLOR LOOKUP ###
    def _setup_hanim_skinned(self, humanoid_node):
        self._build_skeleton_map(humanoid_node)
        coord_node = humanoid_node.skinCoord
        if isinstance(coord_node, x3d.Coordinate) and not coord_node.point and coord_node.USE: coord_node = self.def_map.get(coord_node.USE, coord_node)
        if not coord_node or not coord_node.point: print("Error: Skinned model has no Coordinate data."); return
        ...
        while nodes_to_process:
            ...
            if isinstance(node, x3d.Shape):
                ...
                if isinstance(geom, x3d.IndexedFaceSet) and geom.coordIndex:
                    ...
                    if geom.texCoord and hasattr(geom.texCoord, 'point') and geom.texCoord.point: all_texcoords = np.array(geom.texCoord.point, dtype=np.float32).reshape(-1, 2)
                if node.appearance and node.appearance.texture and isinstance(node.appearance.texture, x3d.ImageTexture):
                    if node.appearance.texture.url: image_url = node.appearance.texture.url[0]
            elif isinstance(node, x3d.Group) and hasattr(node, 'children'): nodes_to_process.extend(node.children)

        if all_texcoords is not None and image_url:
            print(f"Performing manual UV lookup using texture: {image_url}")
            try:
                ...
                for i in range(num_verts):
                    if i < len(all_texcoords):
                        u, v = all_texcoords[i]
                        # Clamp UVs and convert to pixel coordinates
                        # Note: (1-v) is used because image y-coordinates typically start from the top.
                        x = int(np.clip(u, 0.0, 1.0) * (img_width - 1))
                        y = int(np.clip(1.0 - v, 0.0, 1.0) * (img_height - 1))

                        # Sample color and normalize from [0, 255] to [0.0, 1.0]
                        color = img_data[y, x] / 255.0
                        vertex_colors[i] = color
                    else:
                        vertex_colors[i] = (0.5, 0.5, 0.5) # Default grey for missing UVs
            except (FileNotFoundError, IOError) as e:
                print(f"Warning: Could not load texture '{image_url}': {e}")
                vertex_colors = None
        ...
        # Pass the pre-calculated colors to the geometry setup function
    ...
    def _animate_scene_skinned(self, t):
        ...
        for i in range(len(deformed_vertices)):
            ...
            if total_weight > 1e-5:
                normalized_weights = [w / total_weight for w in self.hanim_humanoid.skin_weights[i]]
                for j_idx, weight in zip(self.hanim_humanoid.skin_indices[i], normalized_weights):
    ...
    def _update_anari_vertices(self, deformed_vertices):
        # Re-creating the array each frame is necessary as ANARI arrays are immutable.
        ...
        self.hanim_humanoid.anari_vertex_array = new_vertex_array # Keep a reference
        ...
        for i in range(len(keys) - 1):
            if keys[i] <= fraction < keys[i+1]:
                ...
                if dot > 0.9995: result = q1 + t * (q2 - q1)
                    ...
                    theta_0, sin_theta_0 = np.arccos(dot), np.sin(np.arccos(dot))
    ...
    def _setup_display(self):
        ...
        renderer=self.device.newRenderer("default"); bg_gradient=self.device.newArray(anari.FLOAT32_VEC4,np.array(((.0,.0,.0,1.),(.0,.0,.0,1.)),dtype=np.float32).reshape((2,1,4))); renderer.setParameter('background',anari.ARRAY,bg_gradient); renderer.commitParameters()
    ...
    def _update_frame(self, frame_num):
        elapsed_time=time.time()-self.start_time; self._animate_scene(elapsed_time); self._update_camera(); self.frame.render(); self.image_display.set_data(self.frame.get('channel.color')); return [self.image_display]
...
if __name__ == "__main__":
    ...
    if not os.path.exists(texture_file):
        print(f"ERROR: Texture file '{texture_file}' not found.")
        print("Please download it or create a placeholder file.")
        exit()
    try:
        ...
        #print("Rendering scene from 'conan_23_Aug2025import.py'...")
