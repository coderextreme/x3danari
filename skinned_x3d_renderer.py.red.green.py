# --- START OF FILE skinned_x3d_renderer.py (CORRECTED FOR ANARI ATTRIBUTES) ---
...
class InteractiveAnariRenderer:
    ...
    def _setup_common_geometry(self, vertices, indices, vertex_colors=None):
        ...
        if vertex_colors is not None:
            c_array = self.device.newArray1D(anari.FLOAT32_VEC3, vertex_colors)
            # --- FIX #1: Use the standard generic attribute name "vertex.attribute0"
            geom.setParameter("vertex.attribute0", anari.ARRAY, c_array)

        material = self.device.newMaterial("matte")

        if vertex_colors is not None:
            # --- FIX #2: Tell the material to read from "attribute0"
    ...
    def _setup_hanim_skinned(self, humanoid_node):
        ...
        print("DEBUG: Generating procedural red/green stripes based on vertex Y-position.")
        frequency = 20.0
        vertex_colors = np.array([
            (1.0, 0.0, 0.0) if math.sin(v[1] * frequency) > 0 else (0.0, 1.0, 0.0)
