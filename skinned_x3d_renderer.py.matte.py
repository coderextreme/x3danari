...
class InteractiveAnariRenderer:
    ...
    def _setup_common_geometry(self, vertices, indices, texcoords=None, sampler=None, diffuse=None, emissive=None):
        ...
        if sampler is not None:
            material.setParameter('color', anari.FLOAT32_VEC3, diffuse if diffuse is not None else (1,1,1))
            material.setParameter("map.color", anari.SAMPLER, sampler)
            material.setParameter("map.color.attribute", anari.STRING, "attribute0")
        else:
            material.setParameter('color', anari.FLOAT32_VEC3, diffuse if diffuse is not None else (0.8, 0.8, 0.8))

        material.setParameter('emissive', anari.FLOAT32_VEC3, emissive if emissive is not None else (0,0,0))
    ...
    def _setup_hanim_skinned(self, humanoid_node):
        ...
        all_texcoords = None
        sampler = None
        diffuse_color, emissive_color = None, None
        ...
        if shape_node:
            ...
            if isinstance(geom_node, x3d.IndexedFaceSet) and hasattr(geom_node, 'texCoord') and geom_node.texCoord:
                tex_coord_container = geom_node.texCoord
                actual_tex_coord_node = None

                if isinstance(tex_coord_container, x3d.MultiTextureCoordinate):
                    print("Found MultiTextureCoordinate node. Using first coordinate set.")
                    if tex_coord_container.texCoord:
                        actual_tex_coord_node = tex_coord_container.texCoord[0]
                elif isinstance(tex_coord_container, x3d.TextureCoordinate):
                    print("Found TextureCoordinate node.")
                    actual_tex_coord_node = tex_coord_container
            ...
            if hasattr(shape_node, 'appearance'):
                app_node = shape_node.appearance
                if hasattr(app_node, 'material') and isinstance(app_node.material, x3d.Material):
                    mat_node = app_node.material
                    diffuse_color = getattr(mat_node, 'diffuseColor', (0.8, 0.8, 0.8))
                    emissive_color = getattr(mat_node, 'emissiveColor', (0, 0, 0))
                    print(f"Found Material with diffuseColor: {diffuse_color}")

                if hasattr(app_node, 'texture') and isinstance(app_node.texture, x3d.ImageTexture):
                    tex_node = app_node.texture
                    texture_url = tex_node.url[0] if tex_node.url else None

                    # --- FIX: Handle both file paths and data URIs ---
                    img = None
                    try:
                        if texture_url and texture_url.startswith('data:image'):
                            print("Loading texture from data URI...")
                            header, encoded_data = texture_url.split(',', 1)
                            if ';base64' in header:
                                image_bytes = base64.b64decode(encoded_data)
                                image_stream = io.BytesIO(image_bytes)
                                img = Image.open(image_stream).convert('RGB')
                        elif texture_url and os.path.exists(texture_url):
                            print(f"Loading texture from file '{texture_url}'...")
                            img = Image.open(texture_url).convert('RGB')
                        elif texture_url:
                            print(f"WARNING: Texture '{texture_url}' not found.")
                        ...
                        print(f"Failed to load or process texture: {e}")
                        sampler = None
                    # --- END FIX ---
        ...
        self._setup_common_geometry(all_vertices, all_indices, texcoords=all_texcoords, sampler=sampler, diffuse=diffuse_color, emissive=emissive_color)
...
if __name__ == "__main__":
    # Generic placeholder logic, as the scene file could specify any texture.
    try:
