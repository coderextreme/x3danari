...
class InteractiveAnariRenderer:
    ...
    def _initialize_scene(self):
        ...
        if humanoid_node:
            ...
            if self.surfaces: self.world.setParameterArray('surface', anari.SURFACE, self.surfaces)
        ...
        if not self.lights:
            ...
            light.commitParameters(); self.lights.append(light)
        if self.lights: self.world.setParameterArray('light', anari.LIGHT, self.lights)
        self.world.commitParameters()

    def _traverse(self, node):
        node_def = getattr(node, 'DEF', None)
        if node_def: self.def_map[node_def] = node
        for child_list_name in ['children', 'skeleton', 'skin', 'geometry', 'appearance', 'texture', 'texCoord']:
            child_list = getattr(node, child_list_name, [])
            if not isinstance(child_list, list): child_list = [child_list]
            for child in child_list:
                if child: self._traverse(child)

    def _multiply_quaternions(self, q1, q2):
        x1, y1, z1, w1 = q1; x2, y2, z2, w2 = q2
        w = w1*w2 - x1*x2 - y1*y2 - z1*z2; x = w1*w2 + x1*w2 + y1*z2 - z1*y2
    ...
    def _setup_common_geometry(self, vertices, indices, vertex_colors=None):
        ...
        if vertex_colors is not None:
            print("Applying pre-calculated vertex colors to geometry.")
            color_array = self.device.newArray(anari.FLOAT32_VEC3, vertex_colors)
            geom.setParameter("vertex.color", anari.ARRAY, color_array)
            material.setParameter('color', anari.FLOAT32_VEC3, (1.0, 1.0, 1.0))
        else:
            print("No vertex colors found, applying default material color.")
            material.setParameter('color', anari.FLOAT32_VEC3, (0.6, 0.6, 0.8))
    ...
    def _setup_hanim_skinned(self, humanoid_node):
        ...
        if not coord_node or not hasattr(coord_node, 'point') or not coord_node.point:
            print("Error: Skinned model has no valid Coordinate data."); return
        
        all_vertices = np.array(coord_node.point, dtype=np.float32).reshape(-1, 3)
        all_indices, all_texcoords, image_url, vertex_colors = [], None, None, None

        nodes_to_process = list(humanoid_node.skin)
        while nodes_to_process:
            node = nodes_to_process.pop(0)

            if hasattr(node, 'USE') and node.USE:
                node = self.def_map.get(node.USE, node)

            if isinstance(node, x3d.Shape):
                geom = node.geometry
                if hasattr(geom, 'USE') and geom.USE:
                    geom = self.def_map.get(geom.USE, geom)

                if isinstance(geom, x3d.IndexedFaceSet) and geom.coordIndex:
                    all_indices.extend(parse_x3d_coord_index(geom.coordIndex))
                    
                    # --- FINAL FIX: Handle MultiTextureCoordinate container ---
                    final_tex_coord_node = None
                    if geom.texCoord:
                        tex_coord_container = geom.texCoord
                        if hasattr(tex_coord_container, 'USE') and tex_coord_container.USE:
                            tex_coord_container = self.def_map.get(tex_coord_container.USE, tex_coord_container)

                        if isinstance(tex_coord_container, x3d.MultiTextureCoordinate):
                            print("Found MultiTextureCoordinate, selecting first UV set.")
                            if tex_coord_container.texCoord and len(tex_coord_container.texCoord) > 0:
                                final_tex_coord_node = tex_coord_container.texCoord[0]
                        elif isinstance(tex_coord_container, x3d.TextureCoordinate):
                            final_tex_coord_node = tex_coord_container

                    if final_tex_coord_node:
                        if hasattr(final_tex_coord_node, 'USE') and final_tex_coord_node.USE:
                            final_tex_coord_node = self.def_map.get(final_tex_coord_node.USE, final_tex_coord_node)

                        if hasattr(final_tex_coord_node, 'point') and final_tex_coord_node.point:
                            all_texcoords = np.array(final_tex_coord_node.point, dtype=np.float32).reshape(-1, 2)
                    # --- END FIX ---

                appearance_node = node.appearance
                if hasattr(appearance_node, 'USE') and appearance_node.USE:
                    appearance_node = self.def_map.get(appearance_node.USE, appearance_node)

                if appearance_node and appearance_node.texture:
                    texture_node = appearance_node.texture
                    if hasattr(texture_node, 'USE') and texture_node.USE:
                        texture_node = self.def_map.get(texture_node.USE, texture_node)
                    
                    if isinstance(texture_node, x3d.ImageTexture) and texture_node.url:
                        image_url = texture_node.url[0]

            elif isinstance(node, x3d.Group) and hasattr(node, 'children'):
                nodes_to_process.extend(node.children)

        if all_texcoords is not None and image_url:
            print(f"SUCCESS: Found texture coordinates and image URL. Performing UV lookup...")
            try:
                img_data = load_image_from_data_url(image_url)
                img_height, img_width, _ = img_data.shape
                num_verts = len(all_vertices)
                vertex_colors = np.zeros((num_verts, 3), dtype=np.float32)
                num_uvs = len(all_texcoords)
                print(f"Mapping {num_uvs} UV coordinates to {num_verts} vertices.")
                for i in range(num_verts):
                    if i < num_uvs:
                        u, v = all_texcoords[i]
                        x = int(np.clip(u, 0.0, 1.0) * (img_width - 1))
                        y = int(np.clip(1.0 - v, 0.0, 1.0) * (img_height - 1))
                        color = img_data[y, x] / 255.0
                        vertex_colors[i] = color
                    else:
                        if i == num_uvs: print(f"Warning: More vertices ({num_verts}) than texture coordinates ({num_uvs}). Un-textured vertices will be grey.")
                        vertex_colors[i] = (0.5, 0.5, 0.5)
            except (ValueError, FileNotFoundError, IOError) as e:
                print(f"Warning: Could not load or process texture from URL: {e}")
                vertex_colors = None
        else:
             print("Info: No valid texture coordinates and/or image URL found after checking all USE fields.")

        self._calculate_bind_poses(self.hanim_humanoid.root_joint.DEF, np.identity(4, dtype=np.float32))
        num_verts = len(all_vertices)
        self.hanim_humanoid.skin_indices = [[] for _ in range(num_verts)]; self.hanim_humanoid.skin_weights = [[] for _ in range(num_verts)]
        ...
        self._setup_common_geometry(all_vertices, all_indices, vertex_colors=vertex_colors)
    ...
    def _update_joint_rotations(self, t):
        ...
        for route in self.routes:
            if "fraction_changed" in route.fromField and "set_fraction" in route.toField:
                 interp_def = route.toNode
                 interpolator = self.interpolators.get(interp_def)
                 if interpolator: interpolator.fraction = fraction
            elif "value_changed" in route.fromField and "set_rotation" in route.toField:
                joint_def, interp_def = route.toNode, route.fromNode
                interpolator = self.interpolators.get(interp_def)
                fraction_to_use = getattr(interpolator, 'fraction', fraction)
                if interpolator and joint_def in self.hanim_humanoid.joints:
                    joint_info = self.hanim_humanoid.joints[joint_def]
                    animation_rotation = self._get_interpolated_rotation(interpolator, fraction_to_use)
    ...
    def _get_interpolated_rotation(self, interpolator, fraction):
        ...
        for i in range(len(keys) - 1):
            if keys[i] <= fraction < keys[i+1]:
                ...
                return result / norm if norm > 0 else values[i]
        return values[-1]
    ...
    def _setup_display(self):
        ...
        renderer.setParameter('ambientColor', anari.FLOAT32_VEC3, (0.1, 0.1, 0.1));
    ...
    def _on_press(self, event):
        ...
        self.cam_state.is_dragging=True; self.last_mouse_pos=(event.x,event.y)
...
if __name__ == "__main__":
    try:
        ...
        print(f"Could not import scene file 'walking_man_cc_test.py' ({e}). Ensure it is in the same directory.")
